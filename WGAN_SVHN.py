...
class SVHNDiscriminator(nn.Module):
    ...
    def __init__(self, args):
        ...
        def disc_block(in_channels, out_channels, kernel_size=args.kernel_size, stride=2, padding=1, batch_norm=True):
        ...
        self.disc_model = nn.Sequential(*disc_block(args.im_chan, args.conv_dim, args.kernel_size, batch_norm=False), # 64x64 in, 32x32 out, no batch norm
                                        *disc_block(args.conv_dim, args.conv_dim*2, args.kernel_size), # 16x16 out
                                        *disc_block(args.conv_dim*2, args.conv_dim*4, args.kernel_size), # 8x8 out
                                        *disc_block(args.conv_dim*4, args.conv_dim*8, args.kernel_size), # 4x4 out
                                        )
    ...
    def forward(self, img):
        ...
        output = output.view(-1, args.conv_dim*args.kernel_size*8)
...
class SVHNGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        def gen_block(in_channels, out_channels, kernel_size, stride=args.stride, padding=1, batch_norm=True, relu=True):
        ...
        self.fc = nn.Linear(args.z_size, args.conv_dim*args.kernel_size*args.kernel_size*8)
        self.gen_model = nn.Sequential(*gen_block(args.conv_dim*8, args.conv_dim*4, args.kernel_size),
                                       *gen_block(args.conv_dim*4, args.conv_dim*2, args.kernel_size),
                                       *gen_block(args.conv_dim*2, args.conv_dim, args.kernel_size),
                                       *gen_block(args.conv_dim, args.im_chan, args.kernel_size, batch_norm=False, relu=False),
                                       nn.Tanh()
                                      )
    def forward(self, z):
        # fully connected + reshape
        z = self.fc(z)
        #z = z.view(-1, args.conv_dim*4, 4, 4) # (batch_size, depth, 4, 4)
        z = z.view(-1, args.conv_dim*8, args.kernel_size, args.kernel_size) # (batch_size, depth, 4, 4)
        z = self.gen_model(z)
        return z
# GAN
class SVHNGAN(pl.LightningModule):
    ...
    def critic_loss(self, crit_fake_pred, crit_real_pred, gp, c_lambda):
        '''returns loss of critic given the critic's scores for fake (crit_fake_pred)
        and real (crit_real_pred) images, the gradent penalty (gp), 
        and gradient penalty weight (c_lambda)'''
        crit_loss = torch.mean((crit_fake_pred - crit_real_pred)) + c_lambda * gp
        return crit_loss

    def generator_loss(self, crit_fake_pred):
        '''returns loss of generator iven the critic's scores of the generator'''
    ...
    def get_gradient(self, crit, real, fake, epsilon):
        ...
        mixed_scores = crit(mixed_images) # critic's scores on mixed images
    ...
    def gradient_penalty(self, gradient):
        '''returns the gradient penalty, given a gradient
           Given a batch of image gradients, the magnitude of each image's gradient is calculated 
           and the mean quadratic distance of each magnitude is penalized to 1''' 
        # flatten gradients, each row 1 image
        gradient = gradient.view(len(gradient), -1)
        # magnitude of every row
        gradient_norm = gradient.norm(2, dim=1)
        # penalize mean squared distance of gradient norms from 1
    ...
    def training_step(self, train_batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            # generator loss
            gen_loss = self.generator_loss(self.discriminator(self.generated_imgs))
        ...
        if optimizer_idx == 1:
            ...
            # discrimator loss
            epsilon = torch.rand(len(self.imgs), 1, 1, 1, device=device, requires_grad=True)
            gradient = self.get_gradient(self.discriminator, self.imgs.float(), self.generated_imgs.detach(), epsilon)
            gp = self.gradient_penalty(gradient)
            crit_loss = self.critic_loss(fake_output, real_output, gp, args.c_lambda)
            self.log('crit_loss', crit_loss, on_epoch=True)
...
class SVHNCallbacks(Callback):
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        # save first 10 images of batch every args.save_every epochs
        num_images = 10
        if epoch % args.save_every == 0:
            ...
            self.critic_loss.append(metrics['crit_loss'])
            self.generator_loss.append(metrics['gen_loss'])
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        np.save(os.path.join(args.data_dir, 'fake_images_201e_wgan.npy'), self.fake_images)
        np.save(os.path.join(args.data_dir, 'real_images_201e_wgan.npy'), self.real_images)
        np.save(os.path.join(args.data_dir, 'critic_loss_201e_wgan.npy'), self.real_images)
        np.save(os.path.join(args.data_dir, 'gen_loss_201e_wgan.npy'), self.real_images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ks', '--kernel-size', type=int, default=4)
    parser.add_argument('-ic', '--im-chan', type=int, default=3)
    ...
    parser.add_argument('-cl', '--c-lambda', type=int, default=10)
    # other
    parser.add_argument('-se', '--save-every', type=int, default=10)
